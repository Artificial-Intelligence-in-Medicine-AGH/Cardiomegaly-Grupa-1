...
def analyze_heart_center(mask):
    ...
    # Compute distance transform for the heart mask
    ...
    # Dijkstra path through the center of the heart
    ...
    # Heart area to the left/right of the center line
    ...
    # NOTE: Lower heart corners (below the center and radius) — disabled section
    ...
    for filename in tqdm(os.listdir(folder_path)):
        if filename.lower().endswith((".png", ".jpg", ".jpeg")):
            ...
            if save_plots:
                ...
                if data["path"].size > 0:
                    plt.plot(data["path"][:, 1], data["path"][:, 0], 'orange', label="Dijkstra path")

                plt.plot(data["center_x"], data["center_y"], 'ro', label="Heart center")
                plt.axvline(x=data["center_x"], color='orange', linestyle='-', linewidth=2, label="Vertical center line")
                plt.gca().add_patch(plt.Circle(
                    (data["center_x"], data["center_y"]),
                    data["radius"], color='r', fill=False, linestyle='--', label="Inscribed circle"
                ))
                ...
                if lung_points.size > 0:
                    ...
                    plt.plot([x1, x2], [y_mean, y_mean], color='red', linewidth=2, label="Lung width")

                # Drawing heart width line
                heart_y = int(data["center_y"] + data["radius"] / 2)
                plt.plot([data["heart_left"], data["heart_right"]], [heart_y, heart_y], color='magenta', linewidth=2, label="Heart width")
